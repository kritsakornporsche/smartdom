...
export default function AdminRoomsPage() {
  ...
  const handleSaveRoom = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      tenant_id: (status === 'มีผู้เช่า' || status === 'Occupied') ? (tenantId ? parseInt(tenantId) : null) : null
    };
    ...
    <div className="flex h-screen bg-[#FDFBF7] text-[#3E342B]">
                  ...
                  {filteredRooms.map((room) => (
                    <div key={room.id} className="group border border-[#E5DFD3] rounded-2xl overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 bg-white">
                      ...
                      <div className="p-5">
                        ...
                        <div className="text-sm text-[#A08D74] font-bold mb-3">
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-5">
                          {room.amenities?.map(aId => {
                            ...
                            return amenity ? (
                              <span key={aId} title={amenity.label} className="w-7 h-7 flex items-center justify-center bg-[#8B7355]/5 rounded-lg text-xs grayscale hover:grayscale-0 transition-all cursor-default">
                                {amenity.icon}
                              </span>
                            ) : null;
                          })}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-[#3E342B]/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg max-h-[90vh] overflow-hidden shadow-2xl border border-[#E5DFD3] flex flex-col">
            <div className="px-8 py-6 border-b border-[#E5DFD3] flex justify-between items-center shrink-0">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
              <form onSubmit={handleSaveRoom} className="space-y-6">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-2xl text-xs font-bold border transition-all ${
                          selectedAmenities.includes(amenity.id)
                            ? 'bg-[#8B7355]/10 border-[#8B7355] text-[#8B7355]'
                            : 'bg-[#FAF8F5] border-[#E5DFD3] text-[#A08D74]'
                        }`}
                      >
                ...
                <div>
                  ...
                  <div className="relative aspect-video rounded-3xl border-2 border-dashed border-[#E5DFD3] bg-[#FAF8F5] overflow-hidden group">
                    {imageUrl ? (
                      ...
                      <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer hover:bg-[#8B7355]/5 transition-colors">
                ...
                <div className="pt-6 border-t border-[#E5DFD3] shrink-0">
                  <button 
                    type="submit"
                    disabled={uploading}
                    className="w-full bg-[#8B7355] hover:bg-[#3E342B] text-white font-bold py-5 rounded-2xl transition-all shadow-lg shadow-[#8B7355]/20 disabled:opacity-50"
                  >
                    {editingId ? 'บันทึกการแก้ไข' : 'สร้างห้องพัก'}
                  </button>
                </div>
              </form>
            </div>
