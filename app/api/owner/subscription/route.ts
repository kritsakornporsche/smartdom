...
export async function POST(req: Request) {
  try {
    ...
    const pkgs = await sql`SELECT * FROM packages WHERE id = ${packageId} AND is_active = 1 LIMIT 1`;
    ...
    const regs = await sql`SELECT coins FROM dormitory_registry WHERE id = ${dormId} LIMIT 1`;
    ...
    // 3. Deduct coins
    await sql`UPDATE dormitory_registry SET coins = coins - ${pkg.price} WHERE id = ${dormId}`;

    // 4. Record coin transaction
    await sql`
      INSERT INTO coin_transactions (dormitory_id, type, amount, description)
      VALUES (${dormId}, 'Usage', ${pkg.price}, ${`ซื้อแพ็กเกจ ${pkg.name}`})
    `;
    ...
    if (currentSubs.length > 0) {
      ...
      await sql`UPDATE subscriptions SET status = 'Expired' WHERE dormitory_id = ${dormId}`;
    }
