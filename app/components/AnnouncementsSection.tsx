...
export default function AnnouncementsSection() {
  ...
  if (loading) return (
    <div className="bg-white rounded-3xl p-6 border border-border animate-pulse">
      <div className="h-4 w-32 bg-background rounded mb-4"></div>
      <div className="space-y-3">
        <div className="h-12 bg-background rounded-2xl"></div>
        <div className="h-12 bg-background rounded-2xl"></div>
      </div>
    </div>
  );
    ...
    <>
      <div className="bg-white rounded-[32px] p-6 border border-border shadow-sm">
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="text-[10px] font-bold text-[#A08D74] bg-background px-2 py-0.5 rounded-md">
            {news.length} รายการ
          </span>
        </div>
        ...
        <div className="space-y-4">
          {news.map((item) => (
            <div key={item.id} className="group cursor-pointer" onClick={() => setSelectedNews(item)}>
              <div className="flex gap-4 items-start p-3 rounded-2xl hover:bg-background transition-colors">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="bg-white rounded-[2.5rem] w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
              <div className="p-10">
                 ...
                 <h2 className="text-2xl font-black text-foreground mb-4">{selectedNews.title}</h2>
                 ...
                 <div className="text-sm font-medium text-[#564D44] leading-relaxed bg-background p-8 rounded-3xl border border-border/50 whitespace-pre-wrap">
