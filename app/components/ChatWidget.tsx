          ...
          <div ref={scrollRef} className="flex-1 p-6 overflow-y-auto space-y-4 bg-gray-50/50">
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center px-6">
                ...
                <p className="text-sm font-black uppercase tracking-wider text-muted-foreground">เริ่มการสนทนา</p>
                <p className="text-sm text-muted-foreground/60 italic mt-1">สอบถามข้อมูลเพิ่มเติมกับเจ้าหน้าที่ได้เลย</p>
              </div>
            )}
            {messages.map((msg) => {
               ...
               return (
                  <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] p-4 rounded-2xl text-sm font-bold leading-normal shadow-sm ${isMe ? 'bg-[#8B7355] text-white rounded-tr-none shadow-lg shadow-[#8B7355]/20' : 'bg-white border border-black/5 text-black rounded-tl-none'}`}>
                      {msg.message}
                    </div>
                  </div>
               );
