    ...
    <aside className="w-64 bg-background border-r border-border flex flex-col shrink-0">
      ...
      <div className="h-20 flex items-center px-8 border-b border-border">
        <div className="flex items-center gap-3 group cursor-pointer">
          <div className={`h-9 w-9 flex items-center justify-center rounded-xl text-white font-display font-bold text-base shadow-sm transition-transform group-hover:scale-105 ${subRole === 'technician' ? 'bg-indigo-600' : 'bg-rose-500'}`}>
            S
          </div>
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        ...
        {navItems.length > 0 ? (
          ...
          <div className="px-4 text-xs text-muted-foreground">ไม่มีเมนูเนื่องจากผู้ใช้ไม่มีสิทธิ์ที่ถูกต้อง</div>
        )}
      </nav>
