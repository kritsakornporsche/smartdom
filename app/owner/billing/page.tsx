    ...
    <div className="min-h-screen bg-[#080F1E] p-8 lg:p-12">
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-4xl font-black text-white tracking-tight">การจัดการบิลและค่าเช่า</h1>
            <p className="text-muted-foreground mt-2 font-medium">ตรวจสอบสถานะการชำระเงินและออกใบแจ้งหนี้ให้ผู้เช่า</p>
          </div>
          <div className="flex flex-wrap gap-4">
            ...
            <button 
              ...
              className="bg-[#0F172A] text-white border border-white/20/10 px-8 py-4 rounded-2xl font-bold shadow-lg hover:bg-gray-50 active:scale-95 transition-all flex items-center gap-3 w-fit"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
           <div className="bg-[#0F172A] p-8 rounded-[2.5rem] border border-white/20/10 shadow-sm relative overflow-hidden group">
              ...
              <p className="text-[10px] font-black text-white/50 uppercase tracking-[0.2em] mb-2">ยอดค้างชำระทั้งหมด</p>
           ...
           <div className="bg-[#0F172A] p-8 rounded-[2.5rem] border border-white/20/10 shadow-sm relative overflow-hidden group">
              ...
              <p className="text-[10px] font-black text-white/50 uppercase tracking-[0.2em] mb-2">ยอดรับชำระแล้ว (รวม)</p>
           ...
           <div className="bg-[#3E342B] p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group hidden lg:block">
              <div className="absolute -right-4 -top-4 w-24 h-24 bg-[#0F172A]/5 rounded-full blur-2xl" />
        ...
        <div className="bg-[#0F172A] rounded-[2.5rem] border border-white/20/10 shadow-sm overflow-hidden">
           <div className="p-8 border-b border-[#F3EFE9] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex bg-[#0F172A] p-1 rounded-xl border border-white/20/10 w-fit">
           ...
           <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                 <thead className="bg-[#0F172A]">
                    <tr>
                       <th className="px-8 py-5 text-[10px] font-bold text-white/50 uppercase tracking-widest">ห้อง</th>
                       <th className="px-8 py-5 text-[10px] font-bold text-white/50 uppercase tracking-widest">ชื่อผู้เช่า</th>
                       <th className="px-8 py-5 text-[10px] font-bold text-white/50 uppercase tracking-widest">รายการ</th>
                       <th className="px-8 py-5 text-[10px] font-bold text-white/50 uppercase tracking-widest">จำนวนเงิน</th>
                       <th className="px-8 py-5 text-[10px] font-bold text-white/50 uppercase tracking-widest">กำหนดชำระ</th>
                       <th className="px-8 py-5 text-[10px] font-bold text-white/50 uppercase tracking-widest">สถานะ</th>
                       <th className="px-8 py-5 text-[10px] font-bold text-white/50 uppercase tracking-widest text-center">จัดการ</th>
                    </tr>
                 </thead>
                    ...
                    {loading ? (
                      [...Array(3)].map((_, i) => (
                        <tr key={i} className="animate-pulse">
                          <td colSpan={7} className="px-8 py-6 h-20 bg-[#0F172A]/50" />
                        </tr>
                      ))
                    ) : filteredBills.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="px-8 py-20 text-center text-white/50 font-bold italic">ไม่พบข้อมูลบิลตามเงื่อนไขที่เลือก</td>
                      </tr>
                    ) : filteredBills.map((bill) => (
                      <tr key={bill.id} className="hover:bg-[#0F172A] transition-colors group">
                         <td className="px-8 py-6 font-black text-white">{bill.room_number}</td>
                         <td className="px-8 py-6 text-white/80 font-bold">{bill.tenant_name}</td>
                         <td className="px-8 py-6">
                            <p className="text-sm font-bold text-white">{bill.title}</p>
                            <p className="text-[10px] text-white/50 font-medium">{bill.billing_cycle}</p>
                         </td>
                         <td className="px-8 py-6 font-black text-primary text-lg">฿{Number(bill.amount).toLocaleString()}</td>
                         <td className="px-8 py-6 text-sm font-bold text-white/80">{new Date(bill.due_date).toLocaleDateString('th-TH')}</td>
                         <td className="px-8 py-6">
                            <select 
                              value={bill.status}
      ...
      {isBatchModalOpen && (
        <div className="fixed inset-0 bg-[#3E342B]/40 backdrop-blur-sm z-50 flex items-center justify-center p-6 overflow-y-auto">
           <div className="bg-[#0F172A] rounded-[40px] w-full max-w-xl shadow-2xl animate-in fade-in zoom-in-95 duration-300 border border-white/20/10">
              <div className="bg-[#3E342B] p-10 text-white relative rounded-t-[40px]">
                 ...
                 <div className="flex items-center gap-4 mb-2">
                    <div className="h-10 w-10 bg-amber-400 rounded-xl flex items-center justify-center text-white">
              ...
              <form onSubmit={handleBatchGenerate} className="p-10 space-y-8">
                 <div className="space-y-6">
                    <div>
                       <label className="block text-[10px] font-black text-white/50 uppercase tracking-widest mb-3 ml-1">หัวข้อบิล (ทุกห้อง)</label>
                       <input 
                          type="text"
                          required
                          value={formData.title}
                          onChange={(e) => setFormData({...formData, title: e.target.value})}
                          className="w-full px-6 py-4 bg-[#0F172A] border border-white/20/10 rounded-2xl outline-none focus:ring-2 focus:ring-amber-400 font-bold text-white"
                       />
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       <div>
                          <label className="block text-[10px] font-black text-white/50 uppercase tracking-widest mb-3 ml-1">รอบบิล</label>
                          <input 
                             type="text"
                             required
                 ...
                 <div className="pt-4 flex gap-4">
                    <button type="button" onClick={() => setIsBatchModalOpen(false)} className="flex-1 py-5 text-white/50 font-bold hover:bg-[#0F172A] rounded-3xl transition-all">ยกเลิก</button>
                    <button 
                      type="submit" 
                      disabled={submitting}
                      className="flex-1 py-5 bg-amber-400 text-white font-black rounded-3xl shadow-xl shadow-amber-400/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
                    >
        ...
        <div className="fixed inset-0 bg-[#3E342B]/40 backdrop-blur-sm z-50 flex items-center justify-center p-6 sm:p-10 overflow-y-auto">
           <div className="bg-[#0F172A] rounded-[40px] w-full max-w-xl shadow-2xl animate-in fade-in zoom-in-95 duration-300 border border-white/20/10">
              ...
              <form onSubmit={handleCreateBill} className="p-10 space-y-8">
                 <div className="space-y-6">
                    <div>
                       <label className="block text-[10px] font-black text-white/50 uppercase tracking-widest mb-3 ml-1">เลือกผู้เช่า (ตามเลขห้อง)</label>
                       <select 
                          required
                          value={formData.tenant_id}
                          onChange={(e) => setFormData({...formData, tenant_id: e.target.value})}
                          className="w-full px-6 py-4 bg-[#0F172A] border border-white/20/10 rounded-2xl outline-none focus:ring-2 focus:ring-primary font-bold text-white transition-all"
                       >
                    ...
                    <div>
                       <label className="block text-[10px] font-black text-white/50 uppercase tracking-widest mb-3 ml-1">หัวข้อบิล</label>
                       <input 
                          type="text"
                          required
                          value={formData.title}
                          onChange={(e) => setFormData({...formData, title: e.target.value})}
                          className="w-full px-6 py-4 bg-[#0F172A] border border-white/20/10 rounded-2xl outline-none focus:ring-2 focus:ring-primary font-bold text-white"
                    ...
                    <div>
                       <label className="block text-[10px] font-black text-white/50 uppercase tracking-widest mb-3 ml-1">จำนวนเงินรวม (บาท)</label>
                       <input 
                          type="number"
                          required
                          value={formData.amount}
                          onChange={(e) => setFormData({...formData, amount: e.target.value})}
                          className="w-full px-8 py-5 bg-[#0F172A] border border-white/20/10 rounded-3xl outline-none focus:ring-2 focus:ring-primary font-black text-3xl text-primary"
                          placeholder="0.00"
                       />
                    </div>
                 </div>
                 ...
                 <div className="pt-4 flex gap-4">
                    <button 
                      ...
                      className="flex-1 py-5 text-white/50 font-bold hover:bg-[#0F172A] rounded-3xl transition-all border border-transparent hover:border-white/20/10"
                    >
                       ยกเลิก
                    </button>
                    <button 
                      type="submit" 
                      disabled={submitting}
                      className="flex-1 py-5 bg-primary text-white font-black rounded-3xl shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
                    >
