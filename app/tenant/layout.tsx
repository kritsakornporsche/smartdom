    ...
    <div className="flex h-screen bg-[#FDFBF7] text-[#4A3F35] font-sans">
      ...
      <aside className="w-72 bg-[#F2EFE9] border-r border-[#E5DFD3] flex flex-col hidden md:flex shrink-0">
        ...
        <nav className="flex-1 p-6 space-y-1.5 overflow-y-auto">
          <Link href="/tenant" className="flex items-center gap-3 px-5 py-3 text-[#5A4D41] hover:bg-[#EBE7DF] hover:text-[#3E342B] rounded-2xl font-medium transition-colors">
            <svg className="w-5 h-5 opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
            หน้าหลัก (Home)
          </Link>
          <Link href="/tenant/billing" className="flex items-center gap-3 px-5 py-3 text-[#5A4D41] hover:bg-[#EBE7DF] hover:text-[#3E342B] rounded-2xl font-medium transition-colors">
            <svg className="w-5 h-5 opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            ชำระค่าหอพัก (Billing)
          </Link>
          <Link href="/tenant/maintenance" className="flex items-center gap-3 px-5 py-3 text-[#5A4D41] hover:bg-[#EBE7DF] hover:text-[#3E342B] rounded-2xl font-medium transition-colors">
            <svg className="w-5 h-5 opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <Link href="/tenant/contract" className="flex items-center gap-3 px-5 py-3 text-[#5A4D41] hover:bg-[#EBE7DF] hover:text-[#3E342B] rounded-2xl font-medium transition-colors">
            <svg className="w-5 h-5 opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2" />
            </svg>
            สัญญาเช่า (Contract)
          </Link>
          <Link href="/tenant/move-out" className="flex items-center gap-3 px-5 py-3 text-[#5A4D41] hover:bg-[#EBE7DF] hover:text-[#3E342B] rounded-2xl font-medium transition-colors">
            <svg className="w-5 h-5 opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
            </svg>
            แจ้งย้ายออก (Move Out)
          </Link>
          <Link href="/tenant/chat" className="flex items-center gap-3 px-5 py-3 text-[#5A4D41] hover:bg-[#EBE7DF] hover:text-[#3E342B] rounded-2xl font-medium transition-colors">
            <svg className="w-5 h-5 opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
            </svg>
            ติดต่อเจ้าของหอ (Chat)
          </Link>
          <div className="pt-4 mt-4 border-t border-[#E5DFD3]">
            <Link href="/profile" className="flex items-center gap-3 px-5 py-3 text-[#5A4D41] hover:bg-[#EBE7DF] hover:text-[#3E342B] rounded-2xl font-medium transition-colors">
              <svg className="w-5 h-5 opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
