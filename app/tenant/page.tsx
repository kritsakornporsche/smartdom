  ...
  return (
    <div className="p-8 lg:p-10 hidden-scrollbar">
      <div className="max-w-6xl mx-auto pb-16 space-y-10">
        ...
        <section className="relative overflow-hidden bg-[#0F172A] rounded-[3rem] border border-white/20/10 p-12 shadow-2xl shadow-[#3E342B]/5">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-12">
                ...
                {roomInfo && (
                    <div className="flex gap-4 md:gap-8 flex-wrap justify-center">
                        <div className="bg-[#0F172A] px-10 py-6 rounded-3xl border border-white/20/10 text-center min-w-[160px] transform hover:scale-105 transition-transform">
                            <span className="block text-[10px] font-black uppercase tracking-[0.2em] text-white/50 mb-2">Room</span>
                            <span className="text-3xl font-black text-white">{roomInfo.room_number}</span>
                        </div>
                        <div className="bg-[#0F172A] px-10 py-6 rounded-3xl border border-white/20/10 text-center min-w-[160px] transform hover:scale-105 transition-transform">
                            <span className="block text-[10px] font-black uppercase tracking-[0.2em] text-white/50 mb-2">Floor</span>
                            <span className="text-3xl font-black text-white">{roomInfo.floor}</span>
                        </div>
                        <div className="bg-[#0F172A] px-10 py-6 rounded-3xl border border-white/20/10 text-center min-w-[160px] transform hover:scale-105 transition-transform">
                            <span className="block text-[10px] font-black uppercase tracking-[0.2em] text-white/50 mb-2">Type</span>
                            <span className="text-xl font-black text-white truncate max-w-[120px]">{roomInfo.room_type}</span>
                        </div>
                    </div>
                )}
            </div>
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            <section>
              ...
              <div className="grid gap-6">
                {recentMaintenance.length === 0 ? (
                    <div className="bg-[#0F172A] border-2 border-dashed border-white/20/10 rounded-[2rem] p-12 text-center">
                    ...
                    <div key={maint.id} className="bg-[#0F172A] rounded-[2rem] border border-white/20/10 p-8 shadow-sm hover:shadow-lg transition-all group">
                      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
                        <div className="flex gap-4 items-start">
                            ...
                            <div>
                                <h4 className="font-bold text-white text-lg">{maint.issue_type || 'รายการแจ้งซ่อม'}</h4>
                                <p className="text-xs text-white/50 font-medium flex items-center gap-1.5 mt-1">
                      ...
                      <p className="text-white/80 leading-relaxed italic border-l-4 border-[#F3EFE9] pl-6 mb-2">"{maint.description}"</p>
                    </div>
                ))}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <Link href="/tenant/contract/simulate" className="group bg-[#0F172A] rounded-[2.5rem] border border-white/20/10 p-10 shadow-sm hover:border-primary/30 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 overflow-hidden relative">
                  ...
                  <div className="flex items-start gap-6 relative z-10">
                    ...
                    <div>
                      <h3 className="text-xl font-black text-white mb-2">จำลองสัญญาเช่า</h3>
                      <p className="text-sm text-muted-foreground leading-relaxed font-medium">คำนวณเงินประกันและค่างวดล่วงหน้า เพื่อวางแผนการย้ายเข้าหรืออาศัยอยู่ต่ออย่างมั่นใจ</p>
                    </div>
                  </div>
                </Link>
                
                <Link href="/tenant/maintenance" className="group bg-[#0F172A] rounded-[2.5rem] border border-white/20/10 p-10 shadow-sm hover:border-accent/30 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                  <div className="flex items-start gap-6 relative z-10">
                    ...
                    <div>
                      <h3 className="text-xl font-black text-white mb-2">แจ้งปัญหาการใช้งาน</h3>
                      <p className="text-sm text-muted-foreground leading-relaxed font-medium">พบกระเบื้องแตก น้ำรั่ว หรือไฟดับ? แจ้งเราได้ทันทีผ่านระบบออนไลน์ 24 ชม.</p>
                    </div>
                  </div>
                </Link>
              </div>
