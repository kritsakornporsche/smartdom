...
async function getDashboardData() {
  ...
  if (!session?.user?.email) return { unpaidBills: [], recentMaintenance: [] };
    ...
    <div className="p-10 md:p-16">
      <div className="max-w-7xl mx-auto pb-16 space-y-12">
        ...
        {unpaidBills.length > 0 && (
          <div className="bg-[#FAF3E8] border border-[#E9C46A] rounded-[2.5rem] p-8 shadow-sm flex flex-col md:flex-row items-center justify-between gap-8 transition-all hover:shadow-md">
            <div className="flex gap-6 items-start">
              ...
              <div>
                <h2 className="text-xl font-bold text-[#8B6A2B] mb-1.5">คุณมียอดค้างชำระ ({unpaidBills.length} รายการ)</h2>
                <p className="text-[#8B6A2B]/80 font-medium max-w-xl">
                  ...
                  เพื่อเลี่ยงค่าปรับล่าช้า โดยยอดที่ใกล้ครบกำหนดที่สุดคือวันที่ {new Date(unpaidBills[0].due_date).toLocaleDateString('th-TH')}
                </p>
              </div>
            </div>
