           ...
           <section className="bg-white rounded-[2.5rem] border border-[#E5DFD3] p-8 md:p-10 shadow-lg animate-in fade-in slide-in-from-top-4 duration-500">
             ...
             <form className="grid sm:grid-cols-2 gap-6" onSubmit={(e) => { e.preventDefault(); setShowForm(false); }}>
               <div className="space-y-2">
                  <label className="text-sm font-black uppercase tracking-wider text-[#A08D74]">ประเภทรถ</label>
               ...
               <div className="space-y-2">
                  <label className="text-sm font-black uppercase tracking-wider text-[#A08D74]">ป้ายทะเบียน</label>
               ...
               <div className="space-y-2">
                  <label className="text-sm font-black uppercase tracking-wider text-[#A08D74]">จังหวัด</label>
               ...
               <div className="space-y-2">
                  <label className="text-sm font-black uppercase tracking-wider text-[#A08D74]">ยี่ห้อ / รุ่น</label>
               ...
               <div className="space-y-2">
                  <label className="text-sm font-black uppercase tracking-wider text-[#A08D74]">สีรถ</label>
            ...
            {vehicles.map(v => (
               <div key={v.id} className="bg-white rounded-[2rem] border border-[#E5DFD3] p-8 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 hover:shadow-md transition-shadow">
                 <div className="flex items-center gap-6 w-full md:w-auto">
                    <div className="w-20 h-20 bg-[#FAF8F5] rounded-[1.5rem] flex items-center justify-center text-2xl shadow-inner border border-[#E5DFD3]/50">
                       {v.type === 'รถยนต์' ? '🚗' : '🏍️'}
                    </div>
                    <div>
                       <div className="flex items-center gap-3 mb-1">
                         <h3 className="text-2xl font-black text-[#3E342B]">{v.license_plate}</h3>
                         <span className={`px-3 py-1 text-sm font-black uppercase tracking-wider rounded-full border-2 ${
                       ...
                       <p className="text-sm text-[#A08D74] font-bold uppercase tracking-wider mt-2">
                         {v.brand} • สี{v.color}
                       </p>
                    </div>
                 </div>
