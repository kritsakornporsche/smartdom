...
conn.on('ready', () => {
  ...
  const cmd = 'cd /d D:\\kritsakorn\\smartdom && git pull origin main && (taskkill /F /IM node.exe || echo ok) && ping 127.0.0.1 -n 3 > nul && npm install && rmdir /s /q .next & npm run build && npx pm2 start ecosystem.config.js --update-env && npx pm2 save';
