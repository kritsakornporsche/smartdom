...
conn.on('ready', () => {
  ...
  const cmd = 'cd /d D:\\kritsakorn\\smartdom && git pull origin main && npm install && npx pm2 stop all && (if exist .next rmdir /s /q .next) && npm run build && npx pm2 startOrReload ecosystem.config.js --update-env && npx pm2 save';
