...
conn.on('ready', () => {
  ...
  const cmd = 'cd /d D:\\kritsakorn\\smartdom && git pull origin main && npm install && npx pm2 kill && ping 127.0.0.1 -n 3 > nul && (if exist .next rmdir /s /q .next) && npm run build && npx pm2 start ecosystem.config.js --update-env';
