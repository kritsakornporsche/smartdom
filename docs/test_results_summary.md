...
| ลำดับ | หน้าจอที่ทดสอบ | ขนาดหน้าจอ (Viewport) | ไฟล์ภาพหลักฐาน | คะแนน Lighthouse | LCP | CLS | TBT | ผลการทดสอบการจัดวาง UI |
| :-: | :--- | :--- | :--- | :-: | :-: | :-: | :-: | :--- |
| 1 | **หน้าสำรวจหอพัก (`/explore`)** | Desktop (1440x900) | [`explore_desktop_run1.png`](report_flow/screenshots/explore_desktop_run1.png) | **100 / 100** | 0.70 s | 0.018 | 2 ms | การ์ดหอพักเรียง 3 คอลัมน์ แบนเนอร์คมชัด |
| 2 |  | Mobile (390x844) | [`explore_mobile_run1.png`](report_flow/screenshots/explore_mobile_run1.png) | **96 / 100** | 2.69 s | 0.000 | 82 ms | เมนูย่อเป็น Hamburger การ์ดเรียง 1 คอลัมน์พอดีหน้าจอ |
| 3 | **หน้าเข้าสู่ระบบ (`/signin`)** | Desktop (1440x900) | [`signin_desktop_run1.png`](report_flow/screenshots/signin_desktop_run1.png) | **100 / 100** | 0.76 s | 0.000 | 0 ms | กล่อง Form กึ่งกลางจอ แสง Blur Background สวยงาม |
| 4 |  | Mobile (390x844) | [`signin_mobile_run1.png`](report_flow/screenshots/signin_mobile_run1.png) | **91 / 100** | 3.25 s | 0.000 | 124 ms | Form ขยายเต็มความกว้าง แป้นพิมพ์ไม่บังปุ่ม |
| 5 | **แดชบอร์ดเจ้าของหอ (`/owner`)** | Desktop (1440x900) | [`owner_desktop_run1.png`](report_flow/screenshots/owner_desktop_run1.png) | **97 / 100** | 1.19 s | 0.002 | 0 ms | Sidebar ซ้ายมือ และสถิติ 4 ช่องครบถ้วน (ล็อกอินด้วย Session จริง) |
| 6 |  | Mobile (390x844) | [`owner_mobile_run1.png`](report_flow/screenshots/owner_mobile_run1.png) | **82 / 100** | 4.52 s | 0.000 | 157 ms | Sidebar ซ่อนเป็น Drawer สถิติปรับเป็น 2x2 แถว |
| 7 | **แดชบอร์ดผู้เช่า (`/tenant`)** | Desktop (1440x900) | [`tenant_desktop_run1.png`](report_flow/screenshots/tenant_desktop_run1.png) | **99 / 100** | 0.84 s | 0.012 | 0 ms | แสดงข้อมูลห้อง 101 บิลค้างชำระ และสถานะแจ้งซ่อมครบถ้วน |
| 8 |  | Mobile (390x844) | [`tenant_mobile_run1.png`](report_flow/screenshots/tenant_mobile_run1.png) | **92 / 100** | 3.12 s | 0.001 | 83 ms | ปุ่มกดขนาดใหญ่ (Touch-Friendly) สแกนง่าย |
| 9 | **หน้าระบบบิลและการเงิน (`/bills`)** | Desktop (1440x900) | [`bills_desktop_run1.png`](report_flow/screenshots/bills_desktop_run1.png) | **99 / 100** | 0.96 s | 0.000 | 13 ms | แสดงรายการบิลจริงพร้อมยอดค้างชำระและปุ่ม QR Code ชำระเงิน |
| 10 |  | Mobile (390x844) | [`bills_mobile_run1.png`](report_flow/screenshots/bills_mobile_run1.png) | **88 / 100** | 3.84 s | 0.000 | 135 ms | ตารางปรับเป็นการ์ด Scroll แนวนอนไม่ล้นขอบจอ |

> **หมายเหตุ:** ตัวเลขในตารางนี้คำนวณจากค่ามัธยฐาน (Median) ของการทดสอบจริง 3 รอบต่อหน้า/โหมด ผ่าน URL จริงที่หอพักเกษตร 2 ใช้งาน (`http://kritsakorn.thddns.net:5993`) โดยใช้ Puppeteer ควบคุม Chrome ร่วมกับ Lighthouse User Flow API สำหรับหน้าที่ต้องยืนยันตัวตน (Owner, Tenant, Bills) มีการตั้งค่า Session Cookie ของผู้ใช้จริงลงในเบราว์เซอร์ก่อนวัดผลทุกครั้ง (แก้ไขจากรอบทดสอบก่อนหน้าที่วัดค่าจากหน้าที่ยังไม่ได้ล็อกอิน ทำให้ได้ค่าจากหน้าที่ค้างสถานะกำลังโหลดข้อมูลแทนที่จะเป็นแดชบอร์ดจริง) ยืนยันด้วยภาพหน้าจอที่แนบว่าทุกหน้าแสดงข้อมูลจริงของผู้ใช้ครบถ้วน ไม่มีการ Redirect ไปหน้าเข้าสู่ระบบ

---

## 📊 หมวดที่ 8: การประเมินความพึงพอใจของผู้ใช้งานจริง (User Feedback & Satisfaction)

เก็บผลตอบรับด้วยแบบสอบถามความพึงพอใจแบบมาตราส่วนประมาณค่า (Likert Scale) 5 ระดับ ร่วมกับการสัมภาษณ์แบบไม่มีโครงสร้าง (Unstructured Interview) จากกลุ่มผู้ทดลองใช้งานจริง 4 บทบาท ณ หอพักเกษตร 2 ได้แก่ Owner, Keeper, Tenant และ Platform Admin

เนื่องจากกลุ่มผู้ให้ข้อมูลมีจำนวนจำกัดและคัดเลือกด้วยวิธีเจาะจง (Purposive Sampling) เฉพาะกรณีศึกษาหอพักเกษตร 2 มิได้ใช้การสุ่มตัวอย่างเชิงสถิติ (Non-Probability Sampling) ผลคะแนนเชิงตัวเลขที่ได้จึงมีจำนวนกลุ่มตัวอย่างไม่เพียงพอต่อการสรุปอ้างอิงเป็นค่าเฉลี่ยและส่วนเบี่ยงเบนมาตรฐานในเชิงสถิติที่มีนัยสำคัญ คณะผู้วิจัยจึงเลือกนำเสนอผลในรูปแบบข้อค้นพบเชิงคุณภาพ (Qualitative Findings) แทนการรายงานเป็นค่าสถิติ โดยสรุปจากทิศทางคำตอบและเนื้อหาความคิดเห็นของผู้ให้ข้อมูล ดังนี้

ผู้ทดลองใช้ให้ผลตอบรับในเชิงบวกต่อภาพรวมของระบบในทุกด้านที่ประเมิน ได้แก่ ความง่ายในการใช้งาน การออกแบบหน้าจอ ความสะดวกของระบบออกบิลและ QR พร้อมเพย์ ระบบแจ้งซ่อม ระบบสัญญาดิจิทัล ความรวดเร็วในการประมวลผล และการจัดการสิทธิ์ผู้ใช้งาน โดยไม่มีผู้ให้ข้อมูลรายใดระบุประเด็นเชิงลบหรือข้อกังวลด้านความปลอดภัยระหว่างการทดลองใช้งาน
...
จากการทดสอบระบบ **SmartDom** ณ กรณีศึกษาหอพักเกษตร 2 ครบทั้ง 4 ด้าน พบว่าระบบผ่านเกณฑ์การทดสอบเชิงฟังก์ชันและความปลอดภัย **100% (22/22 Test Cases)** มีประสิทธิภาพการทำงานในระดับดีเยี่ยมตามมาตรฐาน Google Lighthouse (คะแนนเฉลี่ย 94.4/100) ทดสอบครบทั้ง 5 หน้าผ่าน URL จริงที่หอพักเกษตร 2 ใช้งาน พร้อม Session ผู้ใช้ที่ยืนยันตัวตนแล้วสำหรับหน้าที่ต้องล็อกอิน ได้รับผลตอบรับเชิงคุณภาพในทางบวกจากกลุ่มผู้ใช้จริงทุกบทบาท ณ หอพักเกษตร 2 และรองรับการแสดงผลแบบ Responsive เต็มรูปแบบทั้งบนคอมพิวเตอร์ตั้งโต๊ะ (Desktop) และสมาร์ทโฟน (Mobile Viewport) ครอบคลุมการใช้งานจริงในทุกมิติของระบบ จึงพร้อมสำหรับการนำไปใช้งานจริงและการนำเสนอผลการวิจัยในวิทยานิพนธ์อย่างสมบูรณ์
