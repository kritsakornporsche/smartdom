...
function getPool(connectionString) {
  ...
  if (url.startsWith('postgres://') || url.startsWith('postgresql://')) {
    url = 'mysql://smartdom:smartdom@localhost:3306/smartdomdb';
  }
  
  // Fast localhost conversion for local server execution
  if (url.includes('kritsakorn.thddns.net:5994') && process.env.NODE_ENV === 'production') {
    url = url.replace('kritsakorn.thddns.net:5994', 'localhost:3306');
  }
