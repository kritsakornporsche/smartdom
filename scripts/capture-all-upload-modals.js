...
(async () => {
  ...
  // Accept job or open in_progress ticket
  await page.evaluate(() => {
    const cards = Array.from(document.querySelectorAll('.divide-y > div'));
    if (cards[0]) {
      const primaryBtn = cards[0].querySelector('button.bg-primary');
      if (primaryBtn) primaryBtn.click();
    }
  });
  await sleep(2000);

  // Now click "ซ่อมเสร็จแล้ว"
  await page.evaluate(() => {
    const cards = Array.from(document.querySelectorAll('.divide-y > div'));
    for (const card of cards) {
      const doneBtn = card.querySelector('button.bg-emerald-600');
      if (doneBtn) {
        doneBtn.click();
        return;
      }
