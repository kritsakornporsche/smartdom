...
(async () => {
  console.log('🚀 Capturing Meters Checkbox Selection and Action Bar...');
  ...
  // 1. Default View with Checkbox column
  ...
  // 2. Select 3 rows (Room 101, 102, 103)
  await page.evaluate(() => {
    const checkboxes = Array.from(document.querySelectorAll('tbody input[type="checkbox"]'));
    if (checkboxes[0]) checkboxes[0].click();
    if (checkboxes[2]) checkboxes[2].click();
    if (checkboxes[4]) checkboxes[4].click();
  });
  await sleep(1500);

  const savePath2 = path.join(__dirname, '..', 'docs', 'testphoto', 'meters_row_checkboxes_selected.png');
  await page.screenshot({ path: savePath2 });
  console.log(`📸 [SAVED] -> ${savePath2}`);

  // 3. Click "จดมิเตอร์รอบใหม่ (3 ห้อง)" to open modal filtered to 3 rooms
  await page.evaluate(() => {
    const buttons = Array.from(document.querySelectorAll('button'));
    const btn = buttons.find(b => b.textContent && b.textContent.includes('จดมิเตอร์รอบใหม่ ('));
    if (btn) btn.click();
  });
  await sleep(1500);

  const savePath3 = path.join(__dirname, '..', 'docs', 'testphoto', 'meters_batch_selected_rooms_modal.png');
  ...
  console.log('🎉 Done capturing all meters checkbox screens!');
})();
