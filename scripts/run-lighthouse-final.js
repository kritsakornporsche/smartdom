...
// Read cookies from run_lighthouse_final.sh or get directly
...
(async () => {
  console.log('================================================================');
  console.log('🚀 เริ่มต้นการทดสอบ Lighthouse รอบสมบูรณ์ (URL จริง + Auth Cookies)');
  ...
  for (const p of PAGES) {
    for (const m of MODES) {
      for (let run = 1; run <= RUNS_PER_PAGE; run++) {
        ...
        const prefixPath = path.join(REPORT_DIR, prefixName);
        const url = `${BASE_URL}${p.path}`;

        let cookieHeader = '';
        if (p.auth === 'owner') {
          cookieHeader = `--extra-headers="{\\"Cookie\\":\\"${COOKIE_NAME}=${OWNER_COOKIE}\\"}"`;
        } else if (p.auth === 'tenant') {
          cookieHeader = `--extra-headers="{\\"Cookie\\":\\"${COOKIE_NAME}=${TENANT_COOKIE}\\"}"`;
        }

        console.log(`⏳ [${p.name.toUpperCase()}/${m.mode.toUpperCase()}] รอบที่ ${run}/${RUNS_PER_PAGE} (auth=${p.auth}) -> ${url}`);

        const cmd = `npx -y lighthouse "${url}" --output=html,json --output-path="${prefixPath}" --chrome-flags="--headless --no-sandbox" ${m.flags} ${cookieHeader} --quiet`;
        
        try {
          execSync(cmd, { stdio: 'pipe' });
          const jsonPath = `${prefixPath}.report.json`;
          if (fs.existsSync(jsonPath)) {
            const raw = JSON.parse(fs.readFileSync(jsonPath, 'utf-8'));
            const perf = Math.round((raw.categories?.performance?.score || 0) * 100);
            const lcp = ((raw.audits?.['largest-contentful-paint']?.numericValue || 0) / 1000).toFixed(2);
            const cls = (raw.audits?.['cumulative-layout-shift']?.numericValue || 0).toFixed(3);
            const tbt = Math.round(raw.audits?.['total-blocking-time']?.numericValue || 0);
            ...
            console.log(`   ✅ สำเร็จ [Perf: ${perf}, LCP: ${lcp}s, CLS: ${cls}, TBT: ${tbt}ms]`);
          }
        } catch (err) {
  ...
  try {
    ...
    // 2. Direct Screenshot of Live owner and tenant pages to verify Dashboard
    const checkLive = async (targetPath, authCookie, name) => {
      ...
      await page.goto(`${BASE_URL}${targetPath}`, { waitUntil: 'networkidle2', timeout: 25000 }).catch(() => {});
      await sleep(2000);
      const outLivePng = path.join(SCREENSHOT_DIR, `live_${name}_dashboard_verification.png`);
      await page.screenshot({ path: outLivePng });
      console.log(`   📸 ตรวจสอบภาพสด Dashboard: ${outLivePng}`);
      await page.close();
    };
    ...
    await browser.close();
    console.log(`✅ แคปภาพหลักฐานทั้งหมดลงใน ${SCREENSHOT_DIR} เรียบร้อยแล้ว`);
  } catch (err) {
    console.error('⚠️ ข้อผิดพลาดในการแคปภาพ:', err.message);
  }

  console.log('\n================================================================');
  console.log('🎉 การทดสอบ Lighthouse ทั้งหมดเสร็จสิ้นสมบูรณ์!');
  console.log('================================================================');
})();
